...
class SoftMaxLoss {
  public:
    inline dtype loss(PNode x, const vector<dtype> &answer, Metric& eval, int batchsize = 1) {
		...
		vector<dtype> x_val = DEV->to_vector(x->val);
		//DEV->show_val(x->val);
        ...
        for (int i = 0; i < nDim; ++i) {
            if (answer[i] >= 0) {
                if (optLabel < 0 || x_val[i] > x_val[optLabel])
                    optLabel = i;
            }
        }

        dtype sum1 = 0, sum2 = 0, maxScore = x_val[optLabel];
        for (int i = 0; i < nDim; ++i) {
            scores[i] = -1e10;
            if (answer[i] >= 0) {
                scores[i] = exp(x_val[i] - maxScore);
                if (answer[i] == 1)
                    sum1 += scores[i];
                sum2 += scores[i];
            }
        ...
        for (int i = 0; i < nDim; ++i) {
            if (answer[i] >= 0) {
				...
				DEV->set(x->loss, x_loss.data(), nDim);
